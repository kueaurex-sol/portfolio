  ...
  useLayoutEffect(() => {
    ...
    mm.add(
      ...
      (context) => {
        ...
        const ctx = gsap.context(() => {
          // Card 0 sits at its resting spot from the very first frame —
          // no fly-in, no opacity-0 beat. This is what makes the section
          // read as populated the instant it's pinned, instead of the
          // "empty red box" gap you get when every card (including the
          // first) starts hidden and only appears after extra scrolling.
          ...
          // Cards 1..n-1 still fly in and stack on top of card 0 as before.
          cards.forEach((card, i) => {
            if (i === 0) return;
            tl.fromTo(
              card,
        ...
        <div className="relative mx-auto  w-[88vw] px-4 pb-6 lg:h-[42%] sm:w-[80vw] sm:px-6 sm:pb-8 md:h-[40%] h-[36%] lg:w-[70vw] lg:pb-10">
          {offerings.map((o, i) => (
            <div
              key={o.title}
              ref={(el) => (cardRefs.current[i] = el)}
              className="absolute inset-x-4 bottom-0 h-44 overflow-hidden rounded-[22px] p-5 sm:inset-x-6 sm:h-52 sm:rounded-[28px] sm:p-6 lg:h-60 lg:rounded-[32px] lg:p-8"
              style={{ zIndex: i, background: CARD_BG }}
            >
              <span className="mb-1.5 inline-block font-mono text-[10px] tracking-[2px] text-white/70 sm:mb-2 sm:text-xs">
                {String(i + 1).padStart(2, "0")}
