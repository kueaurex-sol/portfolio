...
//   // Since the drawer is now ALWAYS mounted (never conditionally rendered),
//   // there's no exit-animation-complete event to hook into anymore — so
//   // navigation is just deferred a fixed, safely-longer-than-the-close-
//   // animation amount of time instead. 320ms comfortably covers the 260/30
//   // spring transition below.
//   useEffect(() => {
...
//             {/* Was AnimatePresence + conditional mount — same always-
//                 rendered treatment applied here for consistency, since
//                 this is a second spot where mount/unmount + exit
...
//       {/* ---------- Mobile drawer + backdrop ---------- */}
//       {/* No AnimatePresence, no conditional mount/unmount at all — both
//           elements are ALWAYS in the React tree. Only their animated
//           style props change based on mobileOpen. React never has to
//           insert or remove these DOM nodes, so there's nothing for React
//           and framer-motion to race over: framer-motion just interpolates
//           styles on nodes React already owns and never touches again.
//           pointer-events is toggled off when closed so the invisible
//           backdrop/drawer can't intercept clicks/taps meant for the page
//           underneath. */}
//       <motion.div
//         initial={false}
...
//           className="mt-auto rounded-full border border-violet-dim px-5 py-3 font-display text-[13px] tracking-wide text-ink transition-colors hover:border-violet hover:text-violet"
//         >
//           <Link href={'/contact-us'} onClick={(e) => handleNavClick(e, '/contact-us')}>
//             Contact us
//           </Link>
//         </button>
//       </motion.div>
      ...
      <div className="fixed left-1/2 top-1 z-50 hidden -translate-x-1/2 lg:block">
        ...
        >
          <div className="flex h-16 items-center justify-between px-8">
            ...
            {/* Collapses to width 0 (not just opacity 0) when the navbar
                shrinks, so it stops occupying horizontal space in the
                flex row. Previously it stayed full-width-but-invisible,
                which pushed the logo past the container's shrunk width
                and got it clipped by overflow-hidden — and made the
                button look like it had huge left padding since it was
                still being shoved out by the phantom nav width. `layout`
                lets framer-motion smoothly FLIP-animate the width change
                triggered by the w-0/w-auto class toggle below. */}
            <motion.div
              key="nav"
              layout
              initial={false}
              ...
              transition={{
                ...
                opacity: { duration: collapsed ? 0.1 : 0.2, delay: collapsed ? 0 : 0.15 },
              }}
              className={`overflow-hidden whitespace-nowrap font-display text-[13px] tracking-wide ${
