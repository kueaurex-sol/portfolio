        ...
        {CARDS.map((card, i) => {
          ...
          return (
            <div
              ...
              className="h-[54vh] w-screen shrink-0 px-[6vw] md:h-auto md:w-auto md:shrink md:px-0"
            >
              {/* StoryCard-style pop-in — MOBILE ONLY. isMobileRef is only
                  ever true inside the matchMedia("(max-width: 767px)")
                  block above, so on desktop this wrapper renders with NO
                  inline transform/opacity at all, letting TiltCard display
                  normally instead of being stuck forever in its "not
                  entering" (scaled down, faded) resting state — which is
                  what was shrinking/dimming cards 2-4 on desktop before. */}
              <div
                className="h-full w-full md:transform-none md:opacity-100"
                style={
                  isMobileRef.current
                    ? {
